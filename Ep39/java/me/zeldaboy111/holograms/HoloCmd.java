...
public class HoloCmd {
    public static final String prefix = "§6§lHologram§8: §7";
    ...
    private void delete(Player p, String id) {
        ...
        HologramUtil.getInstance().deleteHologram(id);
        p.sendMessage(prefix + "You have deleted the hologram of id §e" + id + "§7.");
        // Work at deleting
    }
    ...
    private void reload(Player p, String id) {
        // no id given -> ""
        if(id == null || id.trim().equals("")) HologramUtil.getInstance().reloadAll(p);
        else if(!HologramUtil.getInstance().checkIfHologramExists(id)) p.sendMessage(prefix + "A hologram with the id §e" + id + " §7does not exist.");
        else HologramUtil.getInstance().reload(id, p);
    }
