...
public class NpcCommand {
    ...
    private void tryExecuteCreate(Player p, String[] args) {
        ...
        else if(NpcUtils.getInstance().doesNpcExist(args[1].toLowerCase())) p.sendMessage(prefix+"A Npc with the id §e" + args[1].toLowerCase() + " §7already exists.");
    ...
    private void tryExecuteDelete(Player p, String[] args) {
        if (args.length <= 1) p.sendMessage(prefix + "Incorrect Usage! §e/npc delete <id>§7.");
            ...
            p.sendMessage(prefix+"A Npc with the id §e" + args[1].toLowerCase() + " §7does not exist.");
