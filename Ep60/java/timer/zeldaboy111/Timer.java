...
public class Timer {
    ...
    public void reset() {
        time = defaultTime;
        Bukkit.getServer().getPluginManager().callEvent(new TimerResetEvent(this));
    }
    ...
    public void start() {
        if(deleted) return;
        manager.add(this);
        Bukkit.getServer().getPluginManager().callEvent(new TimerStartEvent(this));
    }
    ...
    public void stop() {
        if(deleted) return;
        manager.remove(this);
        Bukkit.getServer().getPluginManager().callEvent(new TimerStopEvent(this));
    }
    ...
    /**
     *  This method will delete the timer if existant
     */
    public void delete() {
        if(deleted) return;
        deleted = true;
